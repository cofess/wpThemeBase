<?php
/**
 * Simple Local Avatars of Yiwell WordPress Theme
 * Description:用户自定义头像
 *
 * @package   Yiwell
 * @version   1.0.0
...
/*
如何调用自定义头像？很简单，可以调用get_simple_local_avatar 或 get_avatar 函数即可，例如：
...
class Simple_Local_Avatars {
	...
	public function admin_init() {
		...
		add_settings_field( 'simple-local-avatars-caps', __('Local Avatar Permissions','CS_TEXTDOMAIN'), array( $this, 'avatar_settings_field' ), 'discussion', 'avatars' );
	}
	...
	public function avatar_settings_field( $args ) {
		...
		echo '
			<label for="simple_local_avatars_caps">
				...
				' . __('Only allow users with file upload capabilities to upload local avatars (Authors and above)','CS_TEXTDOMAIN') . '
			</label>
		';
	}
	...
	?>
	<h3><?php _e( 'Avatar','CS_TEXTDOMAIN' ); ?></h3>
 
	<table class="form-table">
		<tr>
			<th><label for="simple-local-avatar"><?php _e('Upload Avatar','CS_TEXTDOMAIN'); ?></label></th>
			...
			<?php
					if ( empty( $profileuser->simple_local_avatar ) )
						echo '<span class="description">' . __('No local avatar is set. Use the upload field to add a local avatar.','CS_TEXTDOMAIN') . '</span>';
					else
						echo '
							<input type="checkbox" name="simple-local-avatar-erase" value="1" /> ' . __('Delete local avatar','CS_TEXTDOMAIN') . '<br />
							<span class="description">' . __('Replace the local avatar by uploading a new avatar, or erase the local avatar (falling back to a gravatar) by checking the delete option.','CS_TEXTDOMAIN') . '</span>
						';
				} else {
					if ( empty( $profileuser->simple_local_avatar ) )
						echo '<span class="description">' . __('No local avatar is set. Set up your avatar at Gravatar.com.','CS_TEXTDOMAIN') . '</span>';
					else
						echo '<span class="description">' . __('You do not have media management permissions. To change your local avatar, contact the blog administrator.','CS_TEXTDOMAIN') . '</span>';
				}
			?>
			</td>
		</tr>
			...
			if ( empty($avatar['file']) ) {		// handle failures
				switch ( $avatar['error'] ) {
					case 'File type does not meet security guidelines. Try another.' :
						add_action( 'user_profile_update_errors', create_function('$a','$a->add("avatar_error",__("Please upload a valid image file for the avatar.","CS_TEXTDOMAIN"));') );
						break;
					default :
						add_action( 'user_profile_update_errors', create_function('$a','$a->add("avatar_error","<strong>".__("There was an error uploading the avatar:","CS_TEXTDOMAIN")."</strong> ' . esc_attr( $avatar['error'] ) . '");') );
				}
