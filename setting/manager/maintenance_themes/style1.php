...
<body class="bg-black parallax">
<div class="container-fluid">
  <div class="row text-center">
    <div class="col-md-6 col-md-offset-3">
      <?php if(cs_get_manager_option('maintenance_logo')){
		  echo '<div class="logo"> <img src="'.cs_get_manager_option('maintenance_logo').'" alt="'.get_bloginfo('name').'"/> </div>';
	  } 
	  if(cs_get_manager_option('maintenance_subject')){
      	echo '<h1 class="subject text-white">'.cs_get_manager_option('maintenance_subject').'</h1>';
      }
	  if(cs_get_manager_option('maintenance_content')){
      	echo '<p class="content text-white">'.cs_get_manager_option('maintenance_content').'</p>';
      }?>
    </div>
  </div>
  <div class="row"> 
    ...
    echo '<div class="col-sm-12 counter">
      <ul class="countdown-timer list-inline text-center text-white" data-countdown="'.cs_get_manager_option('maintenance_completeDate').'">
      </ul>
    </div>';
	}?>
...
<div class="footer">
  ...
  <?php if(cs_get_manager_option('maintenance_footer')){
  	echo '<p class="copyright text-center text-white">'.cs_get_manager_option('maintenance_footer').'</p>';
  }else{
	echo '<p class="copyright text-center text-white">Copyright © '.date('Y').' <a href="'.get_bloginfo('url').'" title="'.get_bloginfo('name').'" class="text-white">'.get_bloginfo('name').'</a> 版权所有</p>';
  }?>
</div>
<div class="fullwidth fixed-top">
  <div class="example-container" id="example-line-container" style="width:<?php echo cs_get_manager_option('maintenance_progress');?>%" data-toggle="tooltip" data-placement="bottom" title="<?php echo cs_get_manager_option('maintenance_progress');?>%"></div>
</div>
