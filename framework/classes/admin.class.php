 ...
 *
 */
class CSAdmin extends CSFramework_Abstract {
  ...
  public function __construct( $settings, $options ) {
    ...
    $this->settings = apply_filters( 'cs_admin_settings', $settings );
    $this->options  = apply_filters( 'cs_admin_options', $options );

    if( ! empty( $this->options ) ) {

      $this->sections   = $this->get_sections();
      $this->get_option = get_option( CS_ADMIN );
      $this->addAction( 'admin_init', 'settings_api' );
      $this->addAction( 'admin_menu', 'admin_menu' );
      //导出设置
      $this->addAction( 'wp_ajax_export_cs_admin_options', 'export' );
  ...
  public static function instance( $settings = array(), $options = array() ) {
    if ( is_null( self::$instance ) && CS_ACTIVE_ADMIN ) {
  ...
  public function export() {
    ...
    header('Content-disposition: attachment; filename=backup-admin-options-'. gmdate( 'd-m-Y' ) .'.txt');
    header('Content-Transfer-Encoding: binary');
    header('Pragma: no-cache');
    header('Expires: 0');

    echo cs_encode_string( get_option( CS_ADMIN ) );
